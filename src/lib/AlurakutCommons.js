...
export function AlurakutMenu({ githubUser }) {
  ...
  const { tema, SelecionaTema } = useContext(AuthContext);

  return (
    <AlurakutMenu.Wrapper isMenuOpen={isMenuOpen} corHeader={tema.corHeader} inputBackGround={tema.inputBackGround} >
      <div className="container">

        <AlurakutMenu.Logo src={`${BASE_URL}/logo.svg`} logoCor={tema.logoCor}/>
        ...
        <nav style={{ alignItems: 'center' }}>
          <span style={{ fontSize: 12, color: '#fff' }}>Tema:</span>

          <a onClick={() => { SelecionaTema(Theme.classico); useState.headerTema = '1' }
          }
            style={{ color: tema.id === "1" && '#000' }}
          >Clássico</a>
          <a onClick={() => { SelecionaTema(Theme.dark); useState.headerTema = '2' }}
            style={{ color: tema.id === "2" && '#000' }}
          >
            Dark</a>
          <div>
...
AlurakutMenu.Wrapper = styled.header`
  width: 100%;
  background-color: ${props => props.corHeader};
...
AlurakutMenu.Logo = styled.img`
  background-color: ${props => props.logoCor};
  padding: 9px 14px;
  border-radius: 1000px;
  height: 34px;
`;
...
export function AlurakutProfileSidebarMenuDefault() {
  ...
  return (
    <AlurakutProfileSidebarMenuDefault.Wrapper texto={tema.texto}>
          ...
          <li key={`orkut__icon_set__${slug}`}>
            ...
            <span className="OrkutNostalgicIconSet__iconComplex" className="OrkutNostalgicIconSet__number" style={{ gridArea: 'number' }}>
              {[0, 1, 2].map((_, index) => {
                ...
                return <img key={`orkut__icon_set__${slug}_img_${index}`} src={`https://alurakut.vercel.app/icons/${icon}.svg`} style={{ marginRight: '2px', opacity: isHeartActive ? 1 : '1' }} />
              })}
            </span>
          </li>
        );
...
// ================================================================================================================
export function ProfileSidebarCommunity() {
  const { tema } = useContext(AuthContext);
  return (
    <ProfileSidebarCommunity.Wrapper texto={tema.texto}>
      <nav>
        <a href="/">
          <img src={`${BASE_URL}/icons/user.svg`} />
          Participar
    ...
    </ProfileSidebarCommunity.Wrapper>
  )
}
ProfileSidebarCommunity.Wrapper = styled.div`
  a {
    font-size: 12px;
    color: ${props => props.texto};
    margin-bottom: 16px;
